...
def flash_message(msg_key):
    # do not move outside of function: needs to reget translation on each call
    # to make sure the correct language is used
    FLASH_MESSAGES = {
...
def subscribe():
    ...
    if email and not errors:
        ...
        success, msg_key = mailchimp_add_subscriber(name, email, tags=["Mailinglist-user"])
        if success:
